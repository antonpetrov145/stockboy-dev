...
class StockBoy:
    ...
    def StoreDictBuilder(self):
        ...
        cursor = mongo.db.stores.find({"owner": session["email"]})
    ...
    def AliasDictBuilder(self):
        ...
        if len(alias_dict) == 0:
            flash(
                Markup(
                    'Please Upload Your Product <a href="https://stockboy.co/dashboard/import/#alias-card">Alias List from Shipstation</a>.'
                )
            )
    ...
    def ReportDictBuilder(self, order_cursor):
        ...
        ##### BY DAY DICTS ######
        by_day_dict = copy.deepcopy(
            session["date_dict"]
        )  ## This does not need to be passed to the session
        ...
        total_inventory = (
            session["inventory_dict"]["meta"]["total_stock"]
            + session["fba_dict"]["meta"]["total_stock"]
        )
        total_stock_value = (
            session["inventory_dict"]["meta"]["stock_value"]
            + session["fba_dict"]["meta"]["stock_value"]
        )
        ...
        session["sales_rank"] = sorted(
            sales_rank.items(), key=lambda kv: kv[1], reverse=True
        )
        session["qty_rank"] = sorted(
            qty_rank.items(), key=lambda kv: kv[1], reverse=True
        )
            ...
            if sku not in fba_dict:
                ...
                if sku in session["inventory_dict"]:
                    if "SB Cost" in session["inventory_dict"][sku]:
                        stock_value += (
                            int(item["InStockSupplyQuantity"])
                            * session["inventory_dict"][sku]["SB Cost"]
                        )
    ...
    def SupplierDictBuilder(self):
        ...
        if count > 0:
            ...
            session["top_bar"]["supplier_average_first_cost"] = (
                average_first_cost / count
            )
    ...
    for order in cursor:
        ...
        for item in order["items"]:
            ...
            if sku in sku_list:
                ...
                if sku == item_sku or item_sku == "All":
                    date_dict[order_date.year][order_date.month][
                        order_date.day
                    ] += sales
...
def FBADictBuilder():
    ...
    for item in cursor:
        ...
        fba_dict[asin]["afn-fulfillable-quantity"] += int(
            item["afn-fulfillable-quantity"]
        )
        fba_dict[asin]["afn-warehouse-quantity"] += int(item["afn-warehouse-quantity"])
        fba_dict[asin]["afn-total-quantity"] += int(item["afn-total-quantity"])
        fba_dict[asin]["afn-inbound-shipped-quantity"] += int(
            item["afn-inbound-shipped-quantity"]
        )
        fba_dict[asin]["afn-reserved-quantity"] += int(item["afn-reserved-quantity"])
...
class SSAPI(FlaskForm):
    user = HiddenField()
    key = TextAreaField(validators.DataRequired())  # current_user.ss_key)
...
class MWSAPI(FlaskForm):
    user = HiddenField()
    merchant = TextAreaField(validators.DataRequired())  # current_user.ss_key)
    key = TextAreaField(validators.DataRequired())  # current_user.ss_key)
    secret = PasswordField(validators.DataRequired())  # current_user.ss_secret)
    submit = SubmitField("Submit")
...
class SupplierSelect(FlaskForm):
    ...
    supplier_id = TextAreaField()
    model_no = TextAreaField()
    lead_time = IntegerField()
    moq = IntegerField()
    notes = TextAreaField()
...
def SSAPIForm(ss_api_form):
    ...
    mongo.db.mongo_user.update(
        {"_id": ObjectId(userid)},
        {
            "$set": {
                "ss_key": ss_api_form.key.data,
                "ss_secret": ss_api_form.secret.data,
            }
        },
        upsert=True,
    )
...
def InventoryFormController(inventoryform):
    ...
    if ws.cell_value(0, 0) != "Inventory Status Report":
        ...
        mongo.db.mongo_user.update(
            {"email": session["email"]}, {"$set": {"inventory_updated": nice_now}}
        )
...
def AliasUploadController(aliasform):
    ...
    if ws.cell_value(0, 2) != "Store Name":
        ...
        mongo.db.mongo_user.update(
            {"email": session["email"]}, {"$set": {"alias_updated": now}}
        )
    ...
    def UserProfile(self):
        ...
        if request.method == "POST" and fbaform.submit.data:
            ...
            mongo.db.mongo_user.update(
                ...
                {
                    "$set": {
                        "fba_merchant_id": fbaform.merchant.data,
                        "fba_access_key": fbaform.key.data,
                        "fba_secret_key": fbaform.secret.data,
                    }
                },
                upsert=True,
            )
...
class Settings(BaseView):
    ...
    def StepThreeView(self):
        ...
        if "aliasupload-file" not in request.files:
            ...
            if session["upload_status"]:
                flash(
                    "Give us about an hour to get your account ready. We'll Email you when it's done."
                )
                flash(
                    "Filling out billing information now is optional, but recommended."
                )
...
class Sales(BaseView):
    ...
    def OrderById(self, orderId):
        ...
        if str(orderId) not in session["orders_dict"].keys():
            # flash('Session cached')
            cursor = mongo.db.orders.find(
                {"$and": [{"orderId": orderId}, {"owner": session["email"]}]}
            )
        ...
        if costform.validate_on_submit():
            ...
            if costform.cost.data != 0.00:
                mongo.db.inventory.update(
                    {"SKU": costform.sku.data},
                    {"$set": {"SB Cost": float(costform.cost.data)}},
                )
                ...
                return jsonify(
                    data={"message": "Cost is {}".format(costform.cost.data)}
                )
...
class ProductView(BaseView):
    ...
    def Product(self, target_sku):
        ...
        for supplier in session["supplier_dict"].values():
            supplierselect.supplier.choices += [
                (str(supplier["_id"]), supplier["name"])
            ]
        ...
        if target_sku in session["sku_sales_dict"]:
            if session["sku_sales_dict"][target_sku]["burn"] > 0:
                session["top_bar"]["days"] = (
                    total_stock / session["sku_sales_dict"][target_sku]["burn"]
                )
            else:
        ...
        return self.render(
            "admin/product_sku.html", sku=target_sku, supplierselect=supplierselect
        )
    ...
    def EditProduct(self):
        ...
        mongo.db.products.update({"_id": id}, {"$set": results})
...
class FBAView(BaseView):
    ...
    def index(self):
        ...
        if total_qty > 0:
            session["top_bar"]["fba_avg_days"] = (
                float(session["fba_dict"]["meta"]["total_stock"]) / float(total_qty)
            ) * default_range
...
class CustomerView(BaseView):
    ...
    def CustomerDetails(self, customer_id):
        ...
        customer_dict = {"customer_value": 0}
        ...
        item_chart, values, shipped_to_amz = ItemChartBuilder(
            customer_search, date_dict, alias_dict, item_sku
        )
        top_bar = []
        ...
        customer_since_date_str = (
            str(customer_since.month)
            + "/"
            + str(customer_since.day)
            + "/"
            + str(customer_since.year)
        )
...
assets_file_dir = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "homepage/assets"
)
...
admin.add_view(
    Sales(
        ...
        menu_icon_type="fa",
        menu_icon_value="fa-sticky-note-o",
    )
)
...
admin.add_view(
    ProductView(
        ...
        endpoint="product",
        menu_icon_type="fa",
        menu_icon_value="fa-shopping-bag",
    )
)
admin.add_view(
    CustomerView(
...
admin.add_view(
    InventoryView(
        ...
        menu_icon_type="fa",
        menu_icon_value="fa-ambulance",
        category="Inventory",
    )
)
admin.add_view(
    FBAView(
        name="FBA", endpoint="fba", menu_icon_type="fa", menu_icon_value="fa-amazon"
    )
)
...
admin.add_view(
    ProfileView(
        name="Import", endpoint="import", menu_icon_type="fa", menu_icon_value="fa-cog"
    )
)
admin.add_view(
    Settings(
